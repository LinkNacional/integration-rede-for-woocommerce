...
if ( ! class_exists('Lkn_Puc_UpdateChecker', false) ) {
    abstract class Lkn_Puc_UpdateChecker {
         ...
         */
        protected function fixSupportedWordpressVersion(Lkn_Puc_Metadata $update = null): void {
            ...
            foreach ($actualWpVersions as $version) {
                if ( preg_match('/^(?P<majorMinor>\d++\.\d++)(?:\.(?P<patch>\d++))?/', $version, $versionParts) ) {
                    if ( $versionParts['majorMinor'] === $update->tested ) {
                        $patch = isset($versionParts['patch']) ? (int) ($versionParts['patch']) : 0;
                        if ( null === $actualWpPatchNumber ) {
         ...
         * @param int $errorType
         */
        public function triggerError($message, $errorType): void {
         ...
         */
        public function collectApiErrors($error, $httpResponse = null, $url = null, $slug = null): void {
         ...
         */
        public function addFilter($tag, $callback, $priority = 10, $acceptedArgs = 1): void {
         ...
         */
        protected function requestMetadata($metaClass, $queryArgs = array()) {
            //Query args to append to the URL.
            $queryArgs = array_merge(
                array(
                    'installed_version' => (string) ($this->getInstalledVersion()),
                    'php' => phpversion(),
                    'locale' => get_locale(),
         ...
         */
        public function clearCachedTranslationUpdates(): void {
